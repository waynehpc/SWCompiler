/*************************************************************************
 *    > File Name: alexnet_cifar.cpp
 *    > Author:  wayne
 *    > mail:
 *    > Created Time: Thu 07 Jan 2021 03:32:01 AM UTC
 ************************************************************************/
...
int main() {
    ...
    auto *data = alexnet->createTensor("data", {MINIBATCH, 32, 32, 3});
    auto *x = alexnet->createConv2d("conv1", data, 96, 3, 1, 1);
    x = alexnet->createRelu("relu1", x);
    x = alexnet->createMaxPool("pool1", x, 2, 2, 0);
    x = alexnet->createLRN("lrn1", x);

    x = alexnet->createConv2d("conv2", x, 256, 3, 1, 1);
    x = alexnet->createRelu("relu2", x);
    x = alexnet->createMaxPool("pool2", x, 2, 2, 0);
