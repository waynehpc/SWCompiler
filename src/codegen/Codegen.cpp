...
void Codegen::initMemoryAllocators() {
    ...
    p_mem_alllocator_ =
        std::make_shared<MemoryAllocator>(cpup, "cpup", 0xFFFFFFFF);
    p_gpumem_allocator_ =
        std::make_shared<MemoryAllocator>(gpup, "gpup", 0xFFFFFFFF);
    ...
    dev_allocator_map_[gpup] = p_gpumem_allocator_.get();

    m_cpu0->setBasePtrName(UniqueName(deviceToStr(cpu0) + "_baseptr"));
    p_mem_alllocator_->setBasePtrName(
        UniqueName(deviceToStr(cpup) + "_baseptr"));

    m_gpu0->setBasePtrName(UniqueName(deviceToStr(gpu0) + "_baseptr"));
    m_gpu1->setBasePtrName(UniqueName(deviceToStr(gpu1) + "_baseptr"));
    p_gpumem_allocator_->setBasePtrName(
        UniqueName(deviceToStr(gpup) + "_baseptr"));
}
...
void Codegen::emitEnvFinalize() { writer_ << "return 0;\n"; }
...
void Codegen::initMakefileBuilder() {
    ...
    if (config_.train_mode) {
        if (!config_.benchmark) {
            ...
            makefile_builder_.addIncDir("/usr/local/protobuf/3.6.1/include");
            makefile_builder_.addLibDir("/usr/local/protobuf/3.6.1/lib");
    ...
    if (config_.mkldnn) {
        makefile_builder_.addIncDir(
            "$(HOME)/install/mkldnn_lnx_1.0.2_cpu_gomp/include");
        makefile_builder_.addLibDir(
            "$(HOME)/install/mkldnn_lnx_1.0.2_cpu_gomp/lib");
...
void Codegen::emitHeader() {
    ...
    if (config_.cuda) {
        ...
        headerWriter_ << "#include \"utils/cudaWrapper.h\"\n";
        headerWriter_ << "#include <nccl.h>\n\n";
    }
    //------------------TODO:END---------------------------------
    // #include "kernels.h"
...
std::string Codegen::generate(std::string outfile) {
    codeGenInit();
    ...
    if(config_.mkldnn)
        writer_ << "std::cout << \"Net with MKLDNN run time \" << (t_end -
    t_begin) << \" ms\\n\";\n"; else writer_ << "std::cout << \"Net run time \"
    << (t_end - t_begin) << \" ms\\n\";\n";
    */
    ...
    std::ofstream fout(outfile, std::fstream::out);
    fout << headerWriter_.get_code() + writer_.get_code();
    fout.close();

    fout.flush();

    if (!config_.sproc_mgpu) {
...
void Codegen::emitMemAllocs() {
    SWLOG_DEBUG(4) << "begin Codegen::emitMemAllocs()\n";
    ...
    SWLOG_DEBUG(4) << "end Codegen::emitMemAllocs()\n";
}
    ...
    for (int i = 0; i < graph->topologyNum(); i++) {
        for (int j = 0; j < graph->getNumInTopoLevel(i); j++) {
            ...
            if (node->nodeType() == TENSOR_NODE) {
                ...
                SWLOG_DEBUG(1)
                    << "allocateMemAddr topo(" << i << ", " << j << ") "
                    << tnode->name() << " " << size << " as " << bufferName
                    << " on dev(" << dev.rank << ", "
...
void Codegen::emitTensorInitializations() {
    ...
    if (config_.train_mode) {
        if (!config_.benchmark) {
            ...
            writer_ << "// emitTensorInitFromSnapshot() disabled in benchmark "
                       "mode\n";
        }
    } else {
    ...
    for (int i = 0; i < graph_->tensorNodeNum(); i++) {
        ...
        case TensorInitType::ZERO:
            writer_ << "addOp(net, \"" << name << "\", " << name << ", " << size
                    << ");"
                    << "\n";
            SWLOG_DEBUG(8) << "[emitSaveSnapshot] " << name
                           << " TensorInitType="
    ...
    */
    for (auto &node : graph_->getDisplayTensorNodes()) {
        ...
        writer_ << "std::cout << \"" << tnode->name() << ":\\n\";"
                << "\n";
        writer_ << "printMatrix(" << tensors_name_map_[out] << ", " << m << ", "
                << n << ");\n";
    }
...
void Codegen::emitDataLoaderInit() {
    SWLOG_DEBUG(4) << "begin Codegen::emitDataLoaderInit()...\n";

    assert(UniqueName("loader") == "loader" &&
           "Error, variable name 'loader' occupied!");
    assert(UniqueName("iter") == "iter" &&
           "Error, variable name 'iter' occupied!");
...
void Codegen::emitInferDataLoaderInit() {
    ...
    writer_ << "std::string " << var_file << " = \"" << config_.dataloader_src
            << "\";\n";
    writer_ << "DataLoader loader(";
    writer_ << var_file << ", ";
...
void Codegen::emitExecute() {
    ...
    if (config_.train_mode) {
        ...
        if (!tensors_name_map_.count(label->getTensor())) {
            SWLOG_DEBUG(4) << "label tensor " << label->name() << " "
                           << label->getTensor() << " not in map ...\n";
            std::cout << "print tensors_name_map_ in" << __FILE__ << " : "
                      << __LINE__ << "\n";
        ...
        if (!tensors_name_map_.count(data->getTensor())) {
            SWLOG_DEBUG(4) << "data tensor " << data->name() << " "
        ...
        */
        ...
        assert(tensors_name_map_.count(label->getTensor()) &&
               "label not allocated");
        assert(tensors_name_map_.count(data->getTensor()) &&
               "data not allocated");
    ...
    if (config_.train_mode) {
        ...
        if (config_.train_config.display) {
            if (!config_.benchmark)
                ...
                writer_ << "// emitPrintGraphOutputs() disabled in benchmark "
                           "mode\n";
            }
        }
...
std::vector<OpNode *> Codegen::getScheduledOpNodes() {
    ...
    */
    ...
    if ((oplabel->getTypeNameLabel()).compare("MatrixMatrixMul") == 0) {
        ...
        if(config_.mkldnn) {
            ...
            emit_mkldnn_memory_desc(in_md, in_dims, in, "any"); // true implies
        format_tag::any emit_mkldnn_memory_desc(w_md, w_dims, w, "any");
            emit_mkldnn_memory_desc(out_md, out_dims, out, "any");
            writer_ << "\n";

            auto fc_desc = name+"_desc";
            writer_ << "auto " << fc_desc << " =
    ...
    if ((oplabel->getTypeNameLabel()).compare("MatrixMatrixFCBias") == 0) {
        ...
        // assert(in->getNDim()==2 && "FC does not support src dim=4
        // currently\n");

        if (config_.mkldnn) {
            /* for mkldnn
             * 1. w already tranposed to oCiC in Engine::transformForMKLDNN()
             * 2. in tensor may be nchw(caffe2, without reshape) | nhwc (swc) |
             * nc
             *
             * */
            auto in_dims = name + "_in_dims";
            ...
            auto out_md = name + "_out_md";
            emit_mkldnn_memory_desc(in_md, in_dims, in,
                                    "any"); // true implies format_tag::any
            ...
            writer_
                ...
                << " = "
                   "inner_product_forward::desc(prop_kind::forward_inference, "
                << in_md << ", " << w_md << ", " << b_md << ", " << out_md
                << ");\n";

            // inner_product primitive descriptor
            auto fc_pd = name + "_pd";
            writer_ << "auto " << fc_pd
                    << " = inner_product_forward::primitive_desc(" << fc_desc
                    << ", " << mkldnn_engine << ");\n";
            writer_ << "\n";
            ...
            auto b_mem = name + "_b_mem";
            auto out_mem = name + "_out_mem";
            emit_mkldnn_memory(in_mem, in, in_dims, mkldnn_engine,
                               tensors_name_map_[in], "nc");
            emit_mkldnn_memory(w_mem, w, w_dims, mkldnn_engine,
                               tensors_name_map_[w], "nc"); // nc
            emit_mkldnn_memory(b_mem, b, b_dims, mkldnn_engine,
                               tensors_name_map_[b]);
            emit_mkldnn_memory(out_mem, out, out_dims, mkldnn_engine,
                               tensors_name_map_[out]);
            writer_ << "\n";

// actual src, weight, dst of mkl inner_product memory
#define EMIT_NEED_REORDER(var, pd, obj, mem)                                   \
    writer_ << "auto " << var << " = " << pd << "." << #obj                    \
            << "_desc() != " << mem << ".get_desc();\n";
            auto need_reorder_in = "need_reorder_" + name + "_in";
            auto need_reorder_w = "need_reorder_" + name + "_w";
            auto need_reorder_out = "need_reorder_" + name + "_out";
...
#define EMIT_REORDER(cmd, flag, from, to)                                      \
    ...
    if ((oplabel->getTypeNameLabel()) == "ElementAdd") {
        ...
        if (config_.mkldnn) {
            // mkldnn support more than 2 source operands with
            // MKLDNN_ARG_MULTIPLE_SRC+i
            auto out_dims = name + "_out_dims";
            ...
            */
            ...
            writer_ << "auto " << sum_pd << " = sum::primitive_desc(" << out_md
                    << ", " << scales_vec << ", " << srcs_vec << ", "
                    << mkldnn_engine << ");\n";
            writer_ << "\n";
            ...
            auto out_mem = name + "_out_mem";
            emit_mkldnn_memory(src0_mem, A, out_dims, mkldnn_engine,
                               tensors_name_map_[A]);
            emit_mkldnn_memory(src1_mem, B, out_dims, mkldnn_engine,
                               tensors_name_map_[B]);
            emit_mkldnn_memory(out_mem, C, out_dims, mkldnn_engine,
                               tensors_name_map_[C]);
    ...
    if ((oplabel->getTypeNameLabel()) == "Conv2d") {
        ...
        if (config_.mkldnn) {
            ...
            auto idims = (input->getMemLayoutTag() == "nhwc")
                             ? input->getShuffledDims(NHWC2NCHW /*{0,3,1,2}*/)
                             : input->getDims();
            ...
            convert_to_c(aalgorithm), &src_desc.data, &weights_desc.data,
            &bias_desc.data, &dst_desc.data, &strides[0], &padding_l[0],
                        &padding_r[0])
            */
            ...
            writer_
                ...
                << "algorithm::convolution_direct, " << in_md << ", " << w_md
                << ", " << b_md << ", " << out_md << ", " << strides_dims
                << ", " << pads_dims << ", " << pads_dims << ");\n";

            // convolution primitive descriptor
            auto conv_pd = name + "_pd";
            writer_ << "auto " << conv_pd
                    << " = convolution_forward::primitive_desc(" << conv_desc
                    << ", " << mkldnn_engine << ");\n";
            writer_ << "\n";
            ...
            auto b_mem = name + "_b_mem";
            auto out_mem = name + "_out_mem";
            emit_mkldnn_memory(in_mem, input, in_dims, mkldnn_engine,
                               tensors_name_map_[input]);
            emit_mkldnn_memory(w_mem, filter, w_dims, mkldnn_engine,
                               tensors_name_map_[filter]);
            emit_mkldnn_memory(b_mem, bias, b_dims, mkldnn_engine,
...
#define CREATE_CONV_MEM(cmem, flag, pd, obj, mem)                              \
    writer_ << "auto " << cmem << " = " << flag << " ? "                       \
            ...
            CREATE_CONV_MEM(weight_mem, need_reorder_w, conv_pd, weights,
                            w_mem);
...
#define EMIT_REORDER(cmd, flag, from, to)                                      \
    ...
    if ((oplabel->getTypeNameLabel()) == "BatchNormalization") {
        ...
        if (config_.mkldnn) {
            ...
            writer_ << "auto " << bn_desc
                    << " = batch_normalization_forward::desc("
                    << "prop_kind::forward_inference, " << in_md << ", "
            ...
            emit_mkldnn_memory(in_mem, input, in_dims, mkldnn_engine,
                               tensors_name_map_[input]);
            // scaleshift_mem actually holds two tensors: scale and bias, so
            // dims is a problem this can be only right when scale and bias are
            // continuous
            writer_ << "auto " << scaleshift_mem << " = memory({{"
                    << scaleshift_dims << "}, "
    ...
    if ((oplabel->getTypeNameLabel()) == "MaxPool" ||
        ...
        if (config_.mkldnn) {
            ...
            std::string pool_type =
                ((oplabel->getTypeNameLabel()) == "MaxPool") ? "max" : "avg";

            auto pool_desc = name + "_desc";
            writer_ << "auto " << pool_desc
                    << " = pooling_forward::desc(prop_kind::forward_inference, "
                    // << "algorithm::pooling_max, " // todo pooling_avg
                    << "algorithm::pooling_" << pool_type << ", " << in_md
                    << ", " << out_md << ", " << strides_dims << ", "
                    << kernel_dims << ", " << pads_dims << ", " << pads_dims
                    << ");\n";

            // poololution primitive descriptor
            auto pool_pd = name + "_pd";
            writer_ << "auto " << pool_pd
                    << " = pooling_forward::primitive_desc(" << pool_desc
                    << ", " << mkldnn_engine << ");\n";
            writer_ << "\n";
    ...
    if ((oplabel->getTypeNameLabel()) == "MaxPoolGrad") {
        ...
        writer_ << "maxpoolGrad_" << dtype_flag << "("
                << tensors_name_map_[inputG] << ", "
                << tensors_name_map_[outputG] << ", "
                << tensors_name_map_[input] << ", " << iDims << ", " << oDims
                << ", " << kernelsVar << ", " << stridesVar << ", " << padsVar
                << ");\n";
    }
    ...
    if ((oplabel->getTypeNameLabel()) == "Relu") {
        ...
        if (config_.mkldnn) {
            ...
            writer_ << "auto " << relu_desc << " = eltwise_forward::desc("
                    ...
                    << "algorithm::eltwise_relu, " << in_md << ", "
                    << "0"
                    << ");\n";

            // BatchNormalizatio primitive descriptor
            auto relu_pd = name + "_pd";
            writer_ << "auto " << relu_pd
                    << " = eltwise_forward::primitive_desc(" << relu_desc
                    << ", " << mkldnn_engine << ");\n";
            writer_ << "\n";
            ...
            emit_mkldnn_memory(in_mem, input, in_dims, mkldnn_engine,
                               tensors_name_map_[input]);
            emit_mkldnn_memory(out_mem, out, in_dims, mkldnn_engine,
                               tensors_name_map_[out]);
            writer_ << "\n";
    ...
    if ((oplabel->getTypeNameLabel()) == "Transpose") {
        ...
        if (config_.mkldnn) {
            ...
            auto odims = (out->getMemLayoutTag() == "nhwc")
                             ? out->getShuffledDims(NHWC2NCHW /*{0,3,1,2}*/)
                             : out->getDims();
    ...
    if ((oplabel->getTypeNameLabel()) == "MatrixTranspose") {
        ...
        if (config_.mkldnn) {
            ...
            auto out_mem = name + "_out_mem";
            emit_mkldnn_memory(in_mem, input, in_dims, mkldnn_engine,
                               tensors_name_map_[input], "cn");
            emit_mkldnn_memory(out_mem, out, out_dims, mkldnn_engine,
                               tensors_name_map_[out], "nc");
            writer_ << "\n";
        ...
        // assert(input->getNDim()==2 && "MatrixTranspose input.dim should be
        // 2");
    ...
    if ((oplabel->getTypeNameLabel()).compare("MatrixSoftmax") == 0) {
        ...
        if (config_.mkldnn) {
            ...
            writer_ << "auto " << softmax_pd
                    << " = softmax_forward::primitive_desc(" << softmax_desc
                    << ", " << mkldnn_engine << ");\n";
            writer_ << "\n";
            ...
            // bn computation
            auto cmd_softmax = name + "_fwd"; // softmax_forward may conflict
                                              // with mkldnn::softmax_forward()
    ...
    if ((oplabel->getTypeNameLabel()).compare("MatrixSoftmaxWithLoss") == 0) {
        ...
        writer_ << "matrixSoftmaxWithLoss_" << dtype_flag << "(" << m << ", "
                << n << ", " << tensors_name_map_[A] << ", " << n << ", "
                << tensors_name_map_[prob] << ", " << n << ", "
                << tensors_name_map_[label] << ", " << tensors_name_map_[loss]
                << ");\n";
    }
    ...
    if ((oplabel->getTypeNameLabel()).compare("MatrixSoftmaxWithLossGrad") ==
        ...
        */
        ...
        writer_ << "matrixSoftmaxWithLossGrad_" << dtype_flag << "(" << m
                << ", " << n << ", " << tensors_name_map_[inputG] << ", " << n
                << ", " << tensors_name_map_[prob] << ", " << n << ", "
                << tensors_name_map_[label] << ");\n";
    }
    ...
    if ((oplabel->getTypeNameLabel()).compare("SGD") == 0) {
        ...
        // auto *input_mirror = ((TensorNode
        // *)op->getChildNode(0))->getTensor();
        ...
        writer_ << "sgd_" << dtype_flag << "(" << size
                ...
                << tensors_name_map_[input] << ", " << tensors_name_map_[input]
                << ", " << tensors_name_map_[inputG] << ", "
...
void Codegen::emitMemFree(std::string name, Device dev) {
    switch (dev.type) {
    case DeviceType::CPU:
        writer_ << "free(" << name << ");\n";
        break;
...
void Codegen::emit_mkldnn_memory_dims(std::string name,
...
void Codegen::emit_mkldnn_memory_desc(std::string &name,
                                      std::string mkldnn_dims, Tensor *tensor,
                                      std::string layout_tag) {
    std::string dtype = getTypeString(tensor);
    assert(dtype_mkldnn_datatype_map.count(dtype) &&
           "mkldnn unsupported data type\n");

    std::string layout =
        layout_tag.length() == 0 ? tensor->getMemLayoutTag() : layout_tag;
...
void Codegen::emit_mkldnn_memory(std::string &name, Tensor *tensor,
                                 std::string mkldnn_dims, std::string engine,
                                 std::string handle, std::string layout_tag) {
    std::string dtype = getTypeString(tensor);
    assert(dtype_mkldnn_datatype_map.count(dtype) &&
           "mkldnn unsupported data type\n");
    ...
    std::string format_tag = layout_tag.length() == 0
                                 ? layout_mkldnn_format_tag_map.at(layout)
                                 : "memory::format_tag::" + layout_tag;

    writer_ << "auto " << name << " = memory({{" << mkldnn_dims << "}, "
            << dtype_mkldnn_datatype_map.at(dtype)
            << ", "
            // << layout_mkldnn_format_tag_map.at(layout) << "}, "
            << format_tag << "}, " << engine << ", " << handle << ");\n";
