...
void Op::checkValid(OpNode *node) {
    ...
    for (i = 0; i < node->getParentNodes().size(); i++) {
        ...
        if (parentIter->getTensor()->getNDim() !=
            ...
            std::cout << "FATAL ERROR: The " << i << "th input tensor "
                      << parentIter->name()
                      << " with dim:" << parentIter->getTensor()->getNDim()
                      << " while the current op " << node->name() << " with "
                      << i << "th"
                      << " dim:" << node->getOp()->getInputDims(i) << std::endl;
            abort();
        }
    }
}
