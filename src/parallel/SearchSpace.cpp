...
std::vector<double> GeneticSearch::getNormAccumFitness() {
    ...
    for (size_t i = 0; i < _populationSize; i++) {
        ...
        fit[i] =
            identity.second > 0 ? identity.second : getFitness(identity.first);
    }
    ...
    for (size_t i = 0; i < _populationSize; i++) {
        fit[i] =
            sum / fit[i]; // caution!!! only did because our fitness is comm
                          // cost(ascend order), but fitness should be time
    }
...
std::vector<int> GeneticSearch::crossover(std::vector<int> &p0,
                                          std::vector<int> &p1) {
...
void GeneticSearch::mutate(std::vector<int> &identity) {
    auto dist =
        std::discrete_distribution<int>{(1 - _mutationRate), _mutationRate};
...
void GeneticSearch::breed() {
    SWLOG_DEBUG(7) << "updating fitness of Population size="
                   << _population.size() << "...\n";
    ...
    std::sort(_population.begin(), _population.end(),
              [](const IdentityWithFit &a, const IdentityWithFit &b) {
    ...
    Population children(_population.begin(),
                        _population.begin() + _numberElites);
    ...
    auto disc_dist =
        std::discrete_distribution<int>{(1 - _crossOverRate), _crossOverRate};

    // select parents to crossover -> child -> mutate
    // roulette method (descend-ordered Population, accumulated fitness, nomal
    // distribution [0,1)
    while (children.size() < _populationSize) {
        auto limit = uniform_dist(rng);
        auto lb1 =
            std::lower_bound(accum_fitness.begin(), accum_fitness.end(), limit);
